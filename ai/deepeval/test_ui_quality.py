...
def build_model() -> OllamaModel:
    load_environment()
    model_name = os.getenv("AI_DEEPEVAL_MODEL", "qwen3:latest")
...
def ensure_ollama_model_installed(base_url: str, model_name: str) -> None:
    ...
    if not has_model:
        raise RuntimeError(
            ...
            f'"{resolved_model}" is not installed. Run: ollama pull {resolved_model}'
        )
...
def artifact_to_summary(artifact: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"Contacto alert: {clean_text(contact.get('alertMessage'))}",
            f"Contacto response: {json.dumps(contact.get('response'), ensure_ascii=False)}",
            f"Login alert: {clean_text(login.get('alertMessage'))}",
            f"Login url: {clean_text(login.get('url'))}",
            f"Monitoring sensor: {json.dumps(monitoring.get('sensor'), ensure_ascii=False)}",
            f"Monitoring actions: {json.dumps(monitoring.get('actions'), ensure_ascii=False)}",
            f"Verify alert: {clean_text(verify.get('alertMessage'))}",
            f"Verify response: {json.dumps(verify.get('response'), ensure_ascii=False)}",
        ]
    )
...
def monitoring_claims(artifact: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"The monitoring page shows connection state: {clean_text(sensor.get('connectionState'))}.",
...
def test_ui_flow_correctness_with_gemini(artifact: dict[str, Any], model: OllamaModel) -> None:
    case = LLMTestCase(
        ...
        expected_output=(
            "The contact form is submitted successfully, login redirects to Mis Plantas, "
            "the first plant is monitored, the Monstera page shows sensor data and polling text, "
            "and manual verification returns a success message."
        ),
    )
...
def test_rag_monitoring_is_faithful_to_documented_ui_context(
    ...
    faithfulness = FaithfulnessMetric(model=model, threshold=0.8, async_mode=False)
    recall = ContextualRecallMetric(model=model, threshold=0.8, async_mode=False)
...
def test_toxicity_is_absent_in_all_captured_ui_messages(
    ...
    metric = ToxicityMetric(model=model, threshold=0.2, async_mode=False)
    score = metric.measure(case, _show_indicator=False)
